...
public class BDTasksManager extends SQLiteOpenHelper {
    ...
    String sqlCreateTaskTable = "CREATE TABLE \"Tasks\" ( \t`task_cve`\tINTEGER PRIMARY KEY AUTOINCREMENT, \t`task_title`\tTEXT, \t`task_content`\tTEXT, \t`task_priority`\tINTEGER, \t`task_begin_date`\tTEXT, \t`task_end_date`\tTEXT, \t`task_id`\tINTEGER, \t`task_latitude`\tREAL, \t`task_longitude`\tREAL, \t`task_status`\tINTEGER, \t`task_user_id`\tINTEGER )";
